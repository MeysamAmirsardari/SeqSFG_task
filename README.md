...
## 13. A separate task: two-tone temporal coherence (`tcoh/`)

`tcoh/` is an **independent experiment** living in this repository, sharing no stimulus code, no
trial schema and no analysis with the stochastic figure-ground work above. It has its own
package, CLI, configs, tests, verification battery, notebook and pre-registration.

**The question.** Elhilali, Ma, Micheyl, Oxenham & Shamma (2009, *Neuron* 61:317–329) argue
that two sounds fuse into one stream when their channels are temporally coherent and split when
they are not. Their Figure 8B makes this quantitative for the simplest possible scene — two
tones — by sweeping onset asynchrony ΔT from exact synchrony to exact alternation and plotting
the model's segregation index λ₂/λ₁, which climbs from 0.01 to 0.93. **That curve has never
been measured behaviourally.** Their own psychophysics (Figure 2) compares only two states, and
through a tempo difference rather than a fixed lag.

**The task.** Two isochronous pure tones, low (A) and high (B), six each, 150 ms SOA, A lagging
B by ΔT. Two such sequences per trial; in one of them the **last B tone is displaced by ±δ**;
2I-2AFC, which one? A 3-down 1-up staircase tracks δ, exactly as in their Figure 2. Thresholds
are normalised between two conditions measured in the same session — ΔT = 0% (one object) and
...
python -m tcoh verify      # 22 checks, no listener needed
python -m tcoh power       # what the design can and cannot detect
...
Three invariants, each checked sample by sample by `python -m tcoh verify`:

1. Within a trial the two intervals are **bit-identical except for one tone's position**.
2. **A never moves**, so the low tone alone carries no information about the answer.
3. **B's grid is identical in every condition of the same length**, so the high tone's own cue
   cannot vary with ΔT.

So the information in either channel *alone* is constant across conditions, and only the
**relation** between them varies with ΔT. A single-channel account of any ΔT effect is excluded
by construction.

### Two things the model decided

`tcoh/model.py` re-implements the paper's coherence analysis for two channels, because the
prediction that matters is the one for *our* stimulus.

*First:* taken as bare arithmetic the published equation does not reproduce the published
numbers — two alternating channels are *anti*-correlated, giving λ₂/λ₁ = 0.16 where the paper
reports 0.93, and a non-monotone curve. Reading step 2 as the **coincidence detection** the
paper's own text describes (a rectified product) reproduces both stated values and the stated
monotonicity. This is an inference about an under-specified method and is flagged as one.

*Second:* **ΔT is only an ordered axis when the tone fills exactly half the period.** At any
other duty cycle the predicted index is not monotone in ΔT — it peaks near 75% and falls again
at full alternation — so a monotone behavioural result would confirm nothing. The validator
refuses other duty cycles.

### The controls, and why H1 settles nothing

Three accounts all predict thresholds rising with ΔT: temporal coherence, interval
discrimination (the judged A–B interval grows with ΔT), and simple acoustic overlap. The
controls separate them. Both hold the **final A–B interval exactly** and remove only the low
tone's *sequence*, so the two rivals — which depend on nothing but the final pair — predict **no
difference at any ΔT**, while coherence predicts a large difference at synchrony that vanishes
at alternation.

Measured by simulating the whole pipeline against listeners generated by each account
(`python -m tcoh power`, 100 sessions per truth):
...
The middle row is the point: the rival produces H1 on every single simulated session. A
shortened design keeps 99% power for H1 and drops to **9%** for H2; the validator says so on
every run.

### Reading further

- [`tcoh/README.md`](tcoh/README.md) — the package, its layout and its limits
- [`tcoh/PREREGISTRATION.md`](tcoh/PREREGISTRATION.md) — hypotheses, decision rules, exclusion
  criteria, power; written before any listener
- [`tcoh/notebooks/tcoh_two_tone_coherence.ipynb`](tcoh/notebooks/tcoh_two_tone_coherence.ipynb)
  — the whole argument with audio and figures
- [`tcoh/verification/`](tcoh/verification/) — battery output, design audit, model checks, figures

Five presets in `tcoh/configs/`: `tcoh_core` (the default, 121 min), `tcoh_full` (209 min; adds a second
control and a build-up manipulation scored against its own ceiling), `tcoh_screen` (short, and *not* a test of the
hypothesis), `tcoh_fine` (nine ΔT levels), and `tcoh_elhilali_replication` (her Figure 2
directly, tempo controls, monaural).
